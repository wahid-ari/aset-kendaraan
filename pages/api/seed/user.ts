  ...
  switch (method) {
    case 'GET':
      if (query.id) {
        ...
        const { data: selectUsers } = await supabase.from('aset_users').select('id').order('id');
        ...
        const { data } = await supabase.from('aset_users').select(`id,name,username,type`).order('id');
        res.status(200).send(JSON.stringify(data, null, 2));
        // res.status(200).json(data);
      }
      break;
